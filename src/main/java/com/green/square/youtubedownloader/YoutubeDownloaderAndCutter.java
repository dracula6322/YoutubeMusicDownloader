...
//        //
  ...
  private ArrayList<Pair<String, String>> getDescFromYoutubeApi(String videoId, String pathToYoutubedl,
...
//    }
  ...
  private File getAudioFile(String id, String outFolder, String name) {
  ...
  private String[] executeFunctionAndGetStringOutput(String stringCommand, ExecutorService inputThread,
      ExecutorService errorThread) {
    return executeFunctionAndGetStringOutputSyncWithRootDir(stringCommand, "", inputThread, errorThread).get(0)
        .toArray(new String[0]);
  }

//  private ArrayList<List<String>> executeFunctionAndGetStringOutputSync(String stringCommand,
//      ExecutorService inputThread, ExecutorService errorThread) {
//
...
//      Process command = runtime.exec(stringCommand, new String[]{}, new File("/"));
//
//      inputThread.execute(() -> {
//        try {
...
//  }
  ...
  private ArrayList<List<String>> executeFunctionAndGetStringOutputSyncWithRootDir(String stringCommand, String rootDir,
      ExecutorService inputThread, ExecutorService errorThread) {
    ...
    try {
      ...
      if (TextUtils.isEmpty(rootDir)) {
        command = runtime.exec(stringCommand);
      } else {
  ...
  private String getFileName(String pathToYoutubedl, String id, ExecutorService inputThread,
      ExecutorService errorThread) {
  ...
  private File downloadFile(String pathToYoutubedl, String id, String saveFolder, String name,
    ...
    ArrayList<List<String>> outputResult = executeFunctionAndGetStringOutputSyncWithRootDir(commandPath, "", inputThread, errorThread);
  ...
  private String downloadJsonInMemory(String pathToYoutubedl, String id, String name,
      ExecutorService inputThread, ExecutorService errorThread) {

    String commandPath = pathToYoutubedl
        + " --skip-download  -f bestaudio "
        + "  -o \"" + "original_%(id)s.%(ext)s\" "
        + "  --print-json "
        + id;

    System.out.println("commandPath = " + commandPath);
    ArrayList<List<String>> result = executeFunctionAndGetStringOutputSyncWithRootDir(commandPath, "", inputThread, errorThread);
  ...
  private ArrayList<String> cutFileByPairs(String ffmpegPath, File audioFile, ArrayList<Pair<String, String>> pairs,
    ...
    for (int i = 0; i < pairs.size(); i++) {
      ...
      if (SystemUtils.IS_OS_LINUX) {
        ...
        rootDirPath = absolutePath + File.pathSeparator;
      } else {
      ...
      //String[] commandPathArray = {"/usr/bin/ffmpeg", "-loglevel", "debug", " -i ", audioFile.getAbsolutePath(), " -ss ", startTime, " -to ", endTime, audioOutName};

      System.out.println("commandPath = " + commandPath);

      //System.out.println("commandPathArray = " + Arrays.toString(commandPathArray));

      ArrayList<List<String>> executeResult = executeFunctionAndGetStringOutputSyncWithRootDir(commandPath, rootDirPath,
